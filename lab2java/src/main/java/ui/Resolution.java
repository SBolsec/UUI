...
public class Resolution {
    ...
    public void resolutionByRefutation(List<Clause> initialClauses, Clause last) {
        ...
        Set<Clause> sos = new HashSet<>(); // set-of-support

        Set<Clause> negatedFinalClause = negateClause(last); // clauses generated by negating the last clause
        sos.addAll(negatedFinalClause); // initialize the sos set

        // initialize the map of resolved clauses
        clauses.forEach(c -> resolved.put(c, new HashSet<>(Arrays.asList(c))));
        sos.forEach(c -> resolved.put(c, new HashSet<>(Arrays.asList(c))));
